...
#' @param baseline_age_col Column name for the Age column.
...
calculate_disease_free_survival <- function(data,
  ...
  working_data <- na.omit(working_data[, c(time_event_col, time_death_col,
                                           event_col, death_col, strata_col,baseline_age_col)])

  # Recalculate times relative to index age
  working_data[[time_event_col]] <- (working_data[[baseline_age_col]] + working_data[[time_event_col]]) - index_age
  working_data[[time_death_col]] <- (working_data[[baseline_age_col]] + working_data[[time_death_col]]) - index_age

  # Drop anyone who had event or was censored before index age
  working_data <- working_data[working_data[[time_event_col]] > 0 &
                                 working_data[[time_death_col]] > 0, ]
